...
object Replica {
  ...
  val insert = send[Insert].receiveAnyOf[OperationAck, OperationFailed]
  val remove = send[Remove].receiveAnyOf[OperationAck, OperationFailed]
  val get = send[Get].receive[GetResult]
  val client = anyOf(insert, remove, get).repeat
